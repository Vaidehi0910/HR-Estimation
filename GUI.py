...
class GUI(QMainWindow, QThread):
    ...
    def initUI(self):
        ...
        self.lblROIforehead = QLabel(self)
        self.lblROIforehead.setGeometry(875,150,110,30)
        self.lblROIforehead.setStyleSheet("background-color: #000000")

        self.lblROIface = QLabel(self)
        self.lblROIface.setGeometry(1000,130,120,80)
        self.lblROIface.setStyleSheet("background-color: #000000")
        ...
        self.lblHR2.setGeometry(875,65,300,40)
        self.lblHR2.setFont(font)
        self.lblHR2.setText("Heart rate: ")

        self.lblHR3 = QLabel(self) #label to show stable HR
        self.lblHR3.setGeometry(900,20,300,40)
        self.lblHR3.setFont(font)
        self.lblHR3.setText("Mask: ")

        self.lblROIforeheadText = QLabel(self)
        self.lblROIforeheadText.setGeometry(895,110,110,30)
        self.lblROIforeheadText.setFont(font2)
        self.lblROIforeheadText.setText("Forehead")

        self.lblROIfaceText = QLabel(self)
        self.lblROIfaceText.setGeometry(1040,100,100,30)
        self.lblROIfaceText.setFont(font2)
        self.lblROIfaceText.setText("Face")
    ...
    def reset(self):
        ...
        self.lblDisplay.setStyleSheet("background-color: #000000")
        self.lblROIface.clear()
        # self.lblROIface.setStyleSheet("background-color: #000000")
        self.lblROIforehead.clear()
    ...
    def main_loop(self):
        ...
        if ret == True:
            ...
            if not self.process.mask:
                self.roi_face = self.process.ROI_face
            self.roi = self.process.ROI_forehead
        ...
        self.roi = np.transpose(self.roi,(0,1,2)).copy()
        f1_img = QImage(self.roi, self.roi.shape[1], self.roi.shape[0], 
                       self.roi.strides[0], QImage.Format_RGB888)
        self.lblROIforehead.setPixmap(QPixmap.fromImage(f1_img))

        if not self.process.mask:
            self.roi_face = cv2.cvtColor(self.roi_face, cv2.COLOR_RGB2BGR)
            self.roi_face = np.transpose(self.roi_face,(0,1,2)).copy()
            f2_img = QImage(self.roi_face, self.roi_face.shape[1], self.roi_face.shape[0], 
                        self.roi_face.strides[0], QImage.Format_RGB888)
            self.lblROIface.setPixmap(QPixmap.fromImage(f2_img))
        
        # self.lblHR.setText("Freq: " + str(float("{:.2f}".format(self.bpm))))
        self.lblHR3.setText("Mask: " + str(self.process.mask))
        ...
        if self.process.bpms.__len__() >50:
            ...
            self.lblHR2.setText("Heart Rate: Processing")
